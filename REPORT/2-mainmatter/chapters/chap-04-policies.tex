...
Seven inventory control policies are implemented in the policies/
package, covering the major families found in the MEIO literature.
They share a common interface --- every policy exposes a single
\texttt{order\_qty} method:
...
The \texttt{t} argument is the simulation day; periodic policies use it
to determine whether a review is due, while continuous-review policies
ignore it.  The \emph{inventory position} (IP) used in most policies is:
...
\textbf{Implementation:} policies/base\_stock.py
...
\textbf{Implementations:} policies/ss\_policy.py,
policies/rq\_policy.py
...
\textbf{Implementations:} policies/order\_up\_to.py,
policies/periodic\_review.py

\subsection{$(R,S)$ periodic-review policy}

The PeriodicReviewPolicy checks inventory every $R$ days and
...
The OrderUpToPolicy generalises the above by allowing a
phase\_offset: reviews occur when $(t - \text{phase\_offset}) \bmod R = 0$.
...
Optional k and m parameters further restrict the
...
\textbf{Implementation:} policies/km\_cycle.py

The KmCyclePolicy schedules at most $k$ replenishment orders per
$m$-day cycle, at pre-specified intra-cycle offsets:

\begin{equation}
  q =
  \begin{cases}
    \max(0, S - \text{IP}) & \text{if } t \bmod m \in review\_offsets
                             \text{ and orders this cycle} < k \\
    0                      & \text{otherwise}
  \end{cases}
\end{equation}

For example, with $m = 7$ and review\_offsets $= \{0, 3\}$, the
...
\textbf{Implementation:} policies/echelon\_stock.py
...
Simulator.\_echelon\_pipeline, and passes the result to the
policy through the pipeline\_in argument.
...
space tractable (25 parameters for 5 SKUs $\times$ 5 non-supplier
nodes — W1, W2, R1, R2, R3) while being flexible enough to demonstrate
the joint optimisation benefit.  The policy comparison experiment (E6,
Section~\ref{sec:results:e6}) re-runs the optimisation with $(s,S)$,
periodic-review, and echelon base-stock as alternatives.
